...
# Configuração de Interface (Paridade Total)
...
if not data.empty:
    ...
    # --- AS 5 LINHAS INSTITUCIONAIS (IDÊNTICAS AO ACD) ---
    ...
    # Fills com Opacidade Sincronizada (0.4)
    ...
    # --- EQUALIZAÇÃO DAS ESCALAS (O PONTO CRÍTICO) ---
    ...
    # Subplot 1: BTC Price Log
    fig.update_yaxes(type="log", row=1, col=1, gridcolor="#222", title="BTC Price")
    
    # Subplot 2: NUPL Equalizado (Números idênticos ao ACD: -3, -2, -1, 0, 1, 2, 3)
    fig.update_yaxes(
        row=2, col=1, 
        gridcolor="#222", # Grelha discreta igual ao ACD
        autorange='reversed', 
        range=[-3.8, 3.8], 
        tickvals=[-3, -2, -1, 0, 1, 2, 3] # Eliminados os números extra
    )
