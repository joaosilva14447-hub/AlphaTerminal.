...
def fetch_nupl_full_sentiment_engine():
    try:
...
data = fetch_nupl_full_sentiment_engine()

if not data.empty:
    last_z = data['z'].iloc[-1]
    
    st.markdown("<h1 style='text-align: center; color: #3D5AFE;'>✦ 𝓑𝓲𝓽𝓬𝓸𝓲𝓷: 𝓝𝓮𝓽 𝓤𝓷𝓻𝓮𝓪𝓵𝓲𝔃𝓮𝓭 𝓟𝓻𝓸𝓯𝓲𝓽/𝓛𝓸𝓼𝓼 ✦</h1>", unsafe_allow_html=True)

    # --- MATRIZ DE SENTIMENTO COMPLETA (Baseada em SDS) ---
    ...
    if last_z >= 3.0:
        ...
        status, s_color = "🔸 ANXIETY / HOPE", "rgba(255, 255, 255, 0.8)"
    elif -1.0 < last_z < 1.0:
    ...
    # --- AJUSTE DE ESCALA (3 SDS COM OVERSHOOT) ---
    fig.update_yaxes(
        row=2, col=1, 
        showgrid=False, 
        autorange='reversed', 
