...
# Configuração de Interface (Sincronizada com ACD)
...
def fetch_anupl_pro():
    try:
        # Motor de Dados com Live Feed (Igual ao ACD para sincronia total)
...
if not data.empty:
    ...
    # Tua Matriz de Sinais Original (Cores Blue/Aqua)
    ...
    # Teu Preenchimento Original (Opacidade 0.7)
    ...
    # AJUSTE FINAL DE ESCALA (AQUI ESTÁ O QUE PEDISTE)
    ...
    # Sincronia de Escala Log do BTC (Igual ao ACD)
    fig.update_yaxes(type="log", row=1, col=1, gridcolor="#222", title="BTC Price (Log)")
    
    # Manutenção da tua Escala Reversa no Indicador
    fig.update_yaxes(row=2, col=1, gridcolor="#222", autorange='reversed', range=[-3.8, 3.8], tickvals=[-3, -2, -1, 0, 1, 2, 3], title="Institutional Sentiment (SD)")
