...
def fetch_nupl_refined_engine():
    try:
        ...
        # --- LÓGICA DE DIFERENCIAÇÃO 180 DIAS ---
        # Foca no limiar entre Short-Term e Long-Term Holder
        data['realized_180'] = data['price'].rolling(window=180).mean()
        
        # Fórmula: $$NUPL = \frac{Price - Realized}{Price}$$
...
data = fetch_nupl_refined_engine()

if not data.empty:
    last_z = data['z'].iloc[-1]
    AQUA, BLUE, WHITE = "#00FBFF", "#3D5AFE", "#FFFFFF"
    ...
    for val, color, dash in [(-3, BLUE, "dot"), (-2, BLUE, "dash"), (3, AQUA, "dot"), (2, AQUA, "dash"), (0, "rgba(255,255,255,0.1)", "solid")]:
