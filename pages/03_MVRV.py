...
def fetch_mvrv_fixed_data():
    try:
        ...
        # 1. Base Fundamental (Realized Proxy)
        data['realized'] = data['price'].rolling(window=365).mean()
        
        # 2. Rácio Logarítmico (Elimina SDS extremas de 5.0)
        data['log_ratio'] = np.log(data['price'] / data['realized'])
        
        # 3. Suavização (Zero Noise)
        data['smooth'] = data['log_ratio'].rolling(window=14).mean()
        
        # 4. Motor Z-Score Invertido (Lógica ACD: Média - Atual)
        window = 350
        data['mean'] = data['smooth'].rolling(window=window).mean()
        data['std'] = data['smooth'].rolling(window=window).std()
        
        # AQUI ESTÁ A CORREÇÃO: (Média - Atual) para que Preço Alto = Z Negativo
        data['z'] = ((data['mean'] - data['smooth']) / data['std']).clip(-3.8, 3.8)
        
        return data.dropna()
    except:
        return pd.DataFrame()

data = fetch_mvrv_fixed_data()

if not data.empty:
    last_z = data['z'].iloc[-1]
    
    st.markdown("<h1 style='text-align: center; color: #3D5AFE;'>✦ 𝓑𝓲𝓽𝓬𝓸𝓲𝓷: 𝓜𝓥𝓡𝓥 𝓩-𝓢𝓬𝓸𝓻𝓮 (𝓕𝓲𝔁𝓮𝓭) ✦</h1>", unsafe_allow_html=True)

    # --- MATRIZ DE SINAIS (Sincronizada com ACD/NUPL) ---
    status, s_color = "NEUTRAL", "#FFFFFF"
    if last_z >= 2.0: 
        status, s_color = "💎 OVERSOLD (BOTTOM)", "#00FBFF"
    elif last_z <= -2.0: 
        status, s_color = "🔴 OVERBOUGHT (TOP)", "#3D5AFE"
    ...
    fig.update_yaxes(
        row=2, col=1, 
        showgrid=False, 
        autorange='reversed', # Inversão visual
        range=[-3.8, 3.8], 
