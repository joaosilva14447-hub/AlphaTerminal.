...
# EWMA realized vol for smoother, more responsive regime
data["vol30"] = data["returns"].ewm(span=30, adjust=False).std() * np.sqrt(365) * 100
data["vol365"] = data["returns"].ewm(span=365, adjust=False).std() * np.sqrt(365) * 100
