...
def fetch_sopr_standard_engine():
    try:
        ...
        # SOPR = Preço de Venda / Preço de Aquisição Médio
...
data = fetch_sopr_standard_engine()
...
if not data.empty:
    ...
    # --- MATRIZ DE SENTIMENTO GRANULAR (Sem Glow) ---
    status, s_color = "NEUTRAL", WHITE
    if last_z >= 2.0: status, s_color = "💎 EXTREME CAPITULATION (BUY)", AQUA
    elif 1.0 <= last_z < 2.0: status, s_color = "🔹 OVERSOLD / PAIN", AQUA
    elif last_z <= -2.0: status, s_color = "🔴 EXTREME EUPHORIA (SELL)", BLUE
    elif -2.0 < last_z <= -1.0: status, s_color = "🔸 OVERBOUGHT / OPTIMISM", BLUE
