...
def fetch_puell_pro_engine():
    try:
        # Download Robusto via Ticker para evitar MultiIndex
        t = yf.Ticker("BTC-USD")
        ...
        # Protocolo de Emissão Bitcoin (Halvings)
        def get_reward(d):
        ...
        # Inversão Alpha: Capitulação (Topo) | Euforia (Base)
...
data = fetch_puell_pro_engine()

if not data.empty:
    last_z = data['z'].iloc[-1]
    
    # --- MATRIZ DE SENTIMENTO E GLOW (Adaptado do SOPR) ---
    ...
    # Título com Glow Dinâmico
    st.markdown(f"""
        <h1 style='text-align: center; color: {BLUE if last_z < 0 else AQUA if last_z > 0 else WHITE}; 
    ...
    # --- FILLS DINÂMICOS (A Lógica Master do SOPR) ---
    ...
    fig.add_trace(go.Scatter(x=data.index, y=np.where(data['z'] <= -2.0, data['z'], -2.0), 
                             fill='tonexty', fillcolor='rgba(61, 90, 254, 0.35)', line=dict(width=0), showlegend=False), row=2, col=1)
    
    # Zona de Capitulação (Aqua)
    fig.add_trace(go.Scatter(x=data.index, y=[2.0]*len(data), line=dict(width=0), showlegend=False), row=2, col=1)
    fig.add_trace(go.Scatter(x=data.index, y=np.where(data['z'] >= 2.0, data['z'], 2.0), 
                             fill='tonexty', fillcolor='rgba(0, 251, 255, 0.35)', line=dict(width=0), showlegend=False), row=2, col=1)
