...
def fetch_supply_shock_engine():
    try:
        ...
        # --- MOTOR DE CÁLCULO: ILLIQUIDITY RATIO ---
        # Medimos a volatilidade real dividida pelo volume relativo
        # Se o preço mexe muito com pouco volume = Supply Shock
        data['returns'] = data['price'].pct_change().abs()
        data['vol_ma'] = data['volume'].rolling(window=20).mean()
        
        # Shock Ratio: Variação de preço por unidade de liquidez
        data['shock_raw'] = data['returns'] / (data['vol_ma'] / data['price'])
        
        # 1. Compressão Logarítmica (Anti-Overfit)
        ...
        # --- INVERSÃO PARA PARIDADE ALPHA ---
        # Z+ (Aqua/Cima) = Choque de Oferta (Bullish Scarcity)
        # Z- (Blue/Baixo) = Excesso de Oferta (Bearish Liquidity)
        data['z'] = ((data['log_shock'] - data['mean']) / data['std']).clip(-3.5, 3.5)
...
data = fetch_supply_shock_engine()
...
if not data.empty:
    ...
    elif 1.0 <= last_z < 2.0: status, s_color = "🔹 LOW LIQUIDITY / AGGRESSIVE", AQUA
    elif last_z <= -2.0: status, s_color = "🔴 SUPPLY OVERLOAD (BEARISH)", BLUE
    elif -2.0 < last_z <= -1.0: status, s_color = "🔸 HIGH LIQUIDITY / DISTRIBUTION", BLUE
    ...
    # Inversão para que Scarcity (Bullish) esteja no topo
    fig.update_yaxes(row=2, col=1, showgrid=False, range=[-3.5, 3.5], tickvals=[-3, -2, -1, 0, 1, 2, 3])
