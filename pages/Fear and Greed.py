...
def state_from_value(value: int):
    """Institutional mapping logic with exact thresholds"""
    if value <= 25:
        return "Extreme Fear", "#00E676"  # Aqua/Green
    elif value <= 40:
        return "Fear", "#00C853"          # Deep Green
    elif value <= 59:
        return "Neutral", "#F5C84B"       # Alpha Yellow
    elif value <= 74:
        return "Greed", "#FF7A45"         # Orange
    else:
        return "Extreme Greed", "#FF3B30" # Red
...
if df_hist is not None:
    ...
    with col1:
        fig_gauge = go.Figure(
            go.Indicator(
                ...
                value=selected_val,
                title={
                    "text": f"{current_row['date_label']} | {selected_status}",
                    "font": {"color": selected_color, "size": 22},
                },
                gauge={
        ...
        fig_gauge.update_layout(paper_bgcolor="rgba(0,0,0,0)", font={"color": "white"}, height=420)
        st.plotly_chart(fig_gauge, use_container_width=True)

    with col2:
        st.markdown("### 24h Sentiment Delta")
        ...
        st.dataframe(
            history_table.iloc[::-1].style.applymap(color_score, subset=["Score"]),
            use_container_width=True,
            hide_index=True
        )

    # --- BOTTOM ROW: IMPROVED HISTORICAL TREND ---
    st.markdown("---")
    st.markdown("### Historical Sentiment Analysis")

    fig_hist = go.Figure()

    # HEATLINE LAYER: Segments for the visual effect
    for i in range(1, len(df_hist)):
        segment_color = df_hist["state_color"].iloc[i]
    ...
    # ACCURACY LAYER: Hidden markers to anchor the correct hover data
    fig_hist.add_trace(go.Scatter(
        x=df_hist["timestamp"],
        y=df_hist["value"],
        mode="markers",
        marker=dict(color="rgba(0,0,0,0)", size=7), # Transparent
        customdata=df_hist["state_label"],
        hovertemplate="<b>%{x|%Y-%m-%d}</b><br>Value: %{y}<br>State: %{customdata}<extra></extra>",
        name="Sentiment"
    ))

    # BACKGROUND THRESHOLD RECTANGLES
    zones = [
        (0, 25, "#00E676", 0.04),
        (25, 40, "#00C853", 0.04),
        (40, 59, "#F5C84B", 0.04),
    ...
    st.error("Terminal offline. Could not fetch Alpha Sentiment data.")

st.markdown("---")
st.caption("Alpha Terminal Institutional Data Feed | Updated every 60m")
